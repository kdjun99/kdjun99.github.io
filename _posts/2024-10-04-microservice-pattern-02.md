...
위 유저 시나리오에서 등장하는 명사들은 클래스들의 힌트가 됩니다.
...
이것이 의미하는 바는 주문 관련 작업을 처리해야하는 애플리케이션의 서비스는 각자마다 `Order` 클래스의 독자적인 도메인 모델을 가진다는 것입니다.
이런 멀티 도메인 모델의 이점의 좋은 예시는 `DeliveryService`입니다.
<img width="758" alt="Screenshot 2024-10-08 at 12 07 28" src="https://github.com/user-attachments/assets/cef024d5-5b50-4561-8b1b-d28a3d591ee7">

앞서 선언한 `Order`클래스와 달리 매우 간단해진 것을 확인할 수 있습니다.
더불어서 `DeliveryService`에서는 `Delivery`라는 보다 더 적합한 이름을 사용할 수 있습니다.
배송 서비스는 배송 이외에 다른 것에는 관심을 두지 않았습니다.

`KitchenService` 역시 주문을 훨씬 간단한 클래스로 다루게 됩니다.
`KitchenService` 에서는 `Ticket`으로 주문 클래스를 다룹니다.
<img width="758" alt="Screenshot 2024-10-08 at 12 10 49" src="https://github.com/user-attachments/assets/274c8e5a-9830-4649-a362-633a25403c2e">

`OrderService` 는 가장 복잡한 구조를 가집니다. 
<img width="758" alt="Screenshot 2024-10-08 at 13 55 28" src="https://github.com/user-attachments/assets/3bd7495f-d748-4e8a-8dfc-f40d37636fd0">

각 도메인 모델의 `Order` 클래스는 주문 비즈니스 엔티티의 다른 측면을 대표합니다.
애플리케이션은 서로 다른 서비스의 서로 다른 오브젝트들간 일관성을 유지해야합니다.
예를 들어 `OrderService`가 고객의 신용 카드를 승인했다면, `KitchenService`의 `Ticket` 생성을 유발해야합니다.
마찬가지로 `KitchenService`가 주문을 거부했따면, `OrderService` 에서도 취소되야합니다.
> 추후에 서비스간 데이터 일관성에 대하여 다룰 예정입니다.

다수의 도메인 모델은 기술적 이슈를 발생시키는 것 뿐만 아니라, ux를 구현하는데도 영향을 미칩니다.
애플리케이션은 사용자 경험과 각 서비스의 도메인 모델 간의 번역을 수행해야 합니다.
예를 들어, 고객에게 보여지는 주문 상태는 다수의 서비스에 저장된 `Order` 정보로부터 도출됩니다.
이 번역은 주로 API 게이트웨이를 통해 처리됩니다.

### defining service APIs

시스템 오퍼레이션과 서비스들을 리스트한 다음 과정은 서비스 API를 정의하는 단계입니다.
서비스 API는 2가지 이유 중 하나로 존재합니다.
일부 작업은 시스템 작업에 해당되고, 외부 클라이언트 또는 다른 서비스에 의해 호출됩니다.
다른 작업들은 서비스간 협업을 지원하기 위해 존재하며, 이러한 작업은 오직 다른 서비스에 의해서만 호출됩니다.

서비스는 주로 서비스간 협업을 활성화하기 위해 이벤트를 발행합니다.
이벤트를 활용해 외부 클라이언트에 알림을 줄 수도 있습니다.
예를 들어, 웹 소켓을 이용해 브라우저에 알림을 주기도 합니다.

서비스 API를 정의하기 위한 첫번째 단계는 시스템 오퍼레이션과 서비스를 연결하는 것입니다.
그후에, 서비스가 시스템 오퍼레이션을 구현하기 위해 다른 서비스와 협업해야하는지 여부를 결정합니다.
만약 협업이 필요하다면, 협업을 지원하기 위해 필요한 API를 결정합니다.

**assigning system operations to services**

첫째로, 요청의 시작 서비스를 결정해야합니다.
많은 시스템 오퍼레이션은 서비스와 깔끔하게 연결되지만, 아닌 경우도 종종 있습니다.
`noteUpdatedLocation` 오퍼레이션이라는 배달원의 위치를 업데이트하는 작업이 `CourierService`에 연결할지, `DeliveryService`에 연결할지 애매합니다.
> 이런 경우 오퍼레이션이 제공하는 정보를 필요로하는 서비스와 연결하는 것이 좋습니다.
> 혹은 오퍼레이션을 처리하기 위한 정보를 가지고 있는 서비스와 연결하는 것도 좋습니다.

<img width="758" alt="Screenshot 2024-10-08 at 14 44 39" src="https://github.com/user-attachments/assets/3217549b-6d86-4159-b67a-c57dc95888b2">

위와 같이 서비스와 오퍼레이션을 연결하고, 각 오퍼레이션을 처리하기 위해 서비스가 어떻게 협업할지를 결정해야 합니다.

어떤 시스템 오퍼레이션은 단일 서비스에서 처리될 수 있습니다.
하지만 몇몇 시스템 오퍼레이션은 다수의 서비스가 필요합니다.
주문 생성 오퍼레이션 같은 경우, `OrderService`가 다음 서비스들을 호출해야합니다.
- ConsumerService : 고객이 주문을 생성할 수 있는지 검증하고 그들의 결제 정보를 리턴 받습니다.
- RestaurantService : 주문 아이템들을 확인하고, 배달 위치, 시간이 식당의 서비스 범위 안에 속하는지 확인하고, 최소 주문 금액을 확인, 주문 금액을 리턴받습니다.
- KitchenService : `Ticket`을 생성합니다.
- AccountingService : 고객의 신용 카드를 승인합니다.

`acceptOrder` 역시 위와 같이 여러 서비스의 협업이 필요합니다.
서비스 API를 완전히 정의하기 위해서는 각 시스템 오퍼레이션을 완전히 분석하고, 어느 서비스 간 협업이 필요한지 결정해야 합니다.

<img width="758" alt="Screenshot 2024-10-08 at 15 04 29" src="https://github.com/user-attachments/assets/effcbd16-41a8-49d7-9dde-f3788d13ef17">

위와 같이 각 서비스가 구현해야할 시스템 오퍼레이션을 식별할 수 있습니다.
현재까지 구성한 아키텍처는 매우 추상적입니다. 어떤 IPC 기술을 사용할지 아무것도 정하지 않았습니다.
> 추후에 여러 IPC 기술들을 다룰 예정입니다.
여러가지 IPC 기술 중 적합한 것을 결정하기 위해 여러 IPC 기술들과 마이크로서비스 패턴들에 대해서 알아야합니다.
여러 마이크로서비스 패턴들에 대해서는 차근차근 다룰 예정입니다.
